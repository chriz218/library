...
public class JwtTokenVerifier extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (!this.jwtService.tokenIsActive(token)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, String.format("Token %s is no longer active, please relogin", token));
        }
        ...
        try {
            ...
            List<Map<String, String>> authoritiesMap = (List<Map<String, String>>) body.get("authorities");

            Set<SimpleGrantedAuthority> simpleGrantedAuthorities = authoritiesMap.parallelStream()
            ...
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, String.format("Token %s has expired, please login again.", token));
        } catch (JwtException e) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, String.format("Token %s cannot be trusted", token));
        }
        filterChain.doFilter(request, response);
    }
}
